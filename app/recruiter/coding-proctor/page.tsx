...
// Add video frame capture utility
const captureVideoFrame = async (videoElement: HTMLVideoElement): Promise<string> => {
...
// Update the FinalResults interface
...
export default function CodingProctor() {
  ...
  const [isCheatingDetectorActive, setIsCheatingDetectorActive] = useState(false)
  ...
  // Add these new functions for fullscreen and security features
  ...
  useEffect(() => {
    if (isTestStarted) {
      ...
      const preventTabSwitch = (e: BeforeUnloadEvent) => {
        e.preventDefault();
        e.returnValue = '';
        return '';
      };
      ...
      document.addEventListener('beforeunload', preventTabSwitch);
      ...
      return () => {
        ...
        document.removeEventListener('beforeunload', preventTabSwitch);
  ...
  const startTest = async () => {
    ...
    // Start camera and cheating detection
    setIsCameraActive(true)
    const detectionCleanup = await startCheatingDetection()
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:11434/api/generate', {
        ...
        body: JSON.stringify({
          model: 'llama3.2',
          prompt: `You are an expert code evaluator. Evaluate the following code solution for the given question.
...
FEEDBACK: [brief explanation of the solution]`,
      ...
      if (!response.ok) {
        throw new Error('Failed to get evaluation from Llama');
      }

      const data = await response.json();
      const evaluationText = data.response.trim();
      const totalScore = parseInt(evaluationText.match(/SCORE: (\d+)/)?.[1] || '0');
      const feedback = evaluationText.match(/FEEDBACK: (.*)/)?.[1] || 'No feedback provided';

      // Get final cheating detection summary
      await stopCheatingDetection();
      const finalCheatingScore = cheatingScore;

      // Calculate adjusted score based on cheating detection
      const adjustedScore = Math.max(0, totalScore * (1 - (finalCheatingScore / 100)));

      // Store the current question's scores
      const currentQuestionScores = {
        question: currentQuestion.question,
        codingScore: adjustedScore,
        mcqScore: 0,
        conceptScore: 0,
        trickyScore: 0,
        totalScore: adjustedScore,
        feedback: feedback,
        cheatingScore: finalCheatingScore,
        cheatingSummary: cheatingSummary
      };

      // Update evaluation metrics
      setEvaluationMetrics({
        correctness: adjustedScore,
        timeComplexity: 0,
        spaceComplexity: 0,
        codeQuality: 0,
        feedback
      ...
      if (adjustedScore >= 25) {
        ...
        if (currentQuestionIndex < selectedQuestions.length - 1) {
          ...
          setFinalResults(prev => ({
            ...
            totalScore: prev.totalScore + adjustedScore
          }));
          setShowResults(true);
      ...
      toast({
        ...
        description: `Score: ${adjustedScore}/50 (Cheating Risk: ${finalCheatingScore}%)`,
      });
    } catch (error) {
  ...
  const evaluateMcqAnswer = async (answer: string, question: string) => {
    ...
    try {
      const response = await fetch('http://localhost:11434/api/generate', {
        ...
        body: JSON.stringify({
          model: 'llama3.2',
          prompt: `You are an expert evaluator. Evaluate the following MCQ answer.
  ...
  const evaluateConceptAnswer = async (answer: string, question: string) => {
    try {
      const response = await fetch('http://localhost:11434/api/generate', {
        ...
        body: JSON.stringify({
          model: 'llama3.2',
          prompt: `You are an expert evaluator. Evaluate the following concept question answer.
...
FEEDBACK: [brief explanation]`,
  ...
  const handleTrickySubmit = () => {
    ...
    if (currentTrickyIndex >= 1 || !currentQuestion?.followUpQuestions?.tricky || currentQuestion.followUpQuestions.tricky.length <= currentTrickyIndex) {
      ...
      // Stop cheating detection and get final summary
      stopCheatingDetection().then(() => {
  ...
  const evaluateTrickyAnswer = async (answer: string, question: string) => {
    try {
      const response = await fetch('http://localhost:11434/api/generate', {
        ...
        body: JSON.stringify({
          model: 'llama3.2',
          prompt: `You are an expert evaluator. Evaluate the following tricky question answer.
...
FEEDBACK: [brief explanation]`,
  ...
  const ResultsPage = () => {
    const calculateTotalScore = () => {
      ...
      // Adjust score based on cheating detection
      const adjustedScore = totalScore * (1 - (cheatingScore / 100));
            ...
            <CardContent>
              <div className="space-y-8">
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="p-6 bg-slate-100 dark:bg-slate-800 rounded-lg">
                    <h3 className="text-xl font-semibold mb-4">Cheating Risk Score</h3>
                    <div className={`text-5xl font-bold ${cheatingScore > 50 ? 'text-red-600' : 'text-green-600'}`}>
                      {cheatingScore}%
                    </div>
                    <p className="text-sm text-slate-500 mt-2">{cheatingStatus}</p>
                  </div>
                </div>

                {/* Add cheating detection summary */}
                {cheatingSummary && (
                  <Card className="border-2 border-red-200">
                    <CardHeader>
                      <CardTitle className="text-xl text-red-600">Cheating Detection Summary</CardTitle>
                    </CardHeader>
                    <CardContent>
  ...
  // Update startCheatingDetection function
  const startCheatingDetection = async () => {
    try {
      ...
      setIsCheatingDetectorActive(true);
      
      // Start processing frames
      const processFrame = async () => {
        if (!isCheatingDetectorActive || !videoRef.current) return;
        ...
        try {
          ...
          if (data.status === 'success') {
            // Update cheating detection results
            setCheatingScore(Math.round(data.score * 100));
            setCheatingStatus(data.risk_level);
            
            // Add event to summary if suspicious activity detected
            if (data.score > 0.5) {
              const event = `Suspicious activity detected: ${data.metrics.head_tilt} head tilt, ${data.metrics.mouth_state} mouth state, ${data.metrics.gaze_direction} gaze`;
              setCheatingSummary(prev => prev + `\n${new Date().toLocaleTimeString()}: ${event}`);
            }
          } else {
            // Handle error cases
            if (data.message === 'No face detected') {
              setCheatingSummary(prev => prev + `\n${new Date().toLocaleTimeString()}: No face detected in frame`);
            } else if (data.message === 'Face absent for too long') {
              setCheatingSummary(prev => prev + `\n${new Date().toLocaleTimeString()}: Face absent for extended period`);
            }
          }
        } catch (error) {
      ...
      return {
        close: () => {
          setIsCheatingDetectorActive(false);
        }
      };
    } catch (error) {
      console.error('Error starting cheating detection:', error);
      toast({
        title: "Error",
        description: "Failed to start cheating detection. Please check server status.",
        variant: "destructive",
      });
      setIsCheatingDetectorActive(false);
      return null;
  ...
  // Update stopCheatingDetection function
  const stopCheatingDetection = async () => {
    try {
      if (isCheatingDetectorActive) {
        // Get final session summary
        const response = await fetch('http://localhost:5000/get-session-summary');
      ...
      console.error('Error stopping cheating detection:', error);
      toast({
        title: "Error",
        description: "Failed to get final summary. Results may be incomplete.",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <main className="md:pl-64 pt-16 md:pt-0">
        ...
        {/* Status indicator for cheating detection */}
        {isCheatingDetectorActive && (
          <div className="fixed bottom-4 right-4 bg-red-500 text-white px-4 py-2 rounded-lg shadow-lg z-50">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 bg-white rounded-full animate-pulse" />
              <span>{cheatingStatus}</span>
            </div>
